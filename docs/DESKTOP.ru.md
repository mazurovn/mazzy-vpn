...
`0750 root:mazzy-vpn`. Bootstrap первого запуска добавляет именной read-only
ACL только для вызвавшего Desktop пользователя и read/write ACL только для
защищённого API socket; публичного доступа к ним нет. Cache не содержат endpoint, пути профилей, приватные
...
В Desktop это по-прежнему partial read-only provider discovery. Отдельно
упакованный Linux `mazzy-agentd` реализует только paired local LAN-WSS egress
capability slice; reverse WSS/iroh, Web/Telegram clients и agent-provider
lifecycle остаются planned. См.
...
добавляется в группу `mazzy-vpn`. Desktop bootstrap дополнительно выдаёт
точечный runtime ACL текущему сеансу, поэтому новый login для немедленной
работы GUI больше не требуется. Package-safe действие «Исправить» повторяет
это добавление, если графический package manager не сохранил сведения о
вызвавшем пользователе.
