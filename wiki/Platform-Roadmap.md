...
| Protocol orchestration | каталог 13, redacted detection/API, read-only planner | typed custom-server import, TUN adapters, authorized failover и leak/rollback tests |
...
| Protocol orchestration | 13-entry catalog, redacted detection/API and read-only planner | typed custom-server import, TUN adapters, authorized failover and leak/rollback tests |
